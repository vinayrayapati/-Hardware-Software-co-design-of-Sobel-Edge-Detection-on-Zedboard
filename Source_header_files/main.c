...
static void EdgeDetectorISR(void *CallBackRef);
...
int main(){
	...
	XScuGic_SetPriorityTriggerType(&IntcInstance,XPAR_FABRIC_EDGE_DETECTOR_0_O_INTR_INTR,0xA0,3);
	status = XScuGic_Connect(&IntcInstance,XPAR_FABRIC_EDGE_DETECTOR_0_O_INTR_INTR,(Xil_InterruptHandler)EdgeDetectorISR,(void *)&myDma);
	if(status != XST_SUCCESS){
		xil_printf("Interrupt connection failed");
		return -1;
	}
	XScuGic_Enable(&IntcInstance,XPAR_FABRIC_EDGE_DETECTOR_0_O_INTR_INTR);
...
static void EdgeDetectorISR(void *CallBackRef){
	static int i=4;
	int status;
	XScuGic_Disable(&IntcInstance,XPAR_FABRIC_EDGE_DETECTOR_0_O_INTR_INTR);
